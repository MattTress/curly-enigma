...
export const ChoiceDeTaiSV = (props) => {
  const {
    handleDeleteTopic,
    handleClose,
    handleSubmit,
  ...
  const [dataDeTaiShow, setDataDeTaiShow] = useState([]);
  ...
  const handleData = () => {
    const calTotalPage = Math.ceil(dataDeTai.length / size);
    setTotalPage(calTotalPage);
    const dataPagination = dataDeTai.slice((page - 1) * size, page * size);
    setDataDeTaiShow(dataPagination);
  };
